...
export function endlessKillShardReward(floor: number): number {
  return Math.min(Math.max(0, Math.floor(floor)) * 2, 20);
}

export function endlessFloorClearBonus(floor: number): number {
  return 8 + Math.max(0, Math.floor(floor));
}
...
export function resolveEndlessAffixBonusCount(floor: number): number {
