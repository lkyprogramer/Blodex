...
interface TalentLegacyMapEntry {
  perRank: number;
  maxRank: number;
}

const LEGACY_TALENT_TO_UPGRADE: Record<string, Partial<PermanentUpgrade>> = {
...
const LEGACY_TALENT_COST: Record<string, number> = {
...
const LEGACY_UPGRADE_TO_TALENT: Record<keyof PermanentUpgrade, TalentLegacyMapEntry & { talentId: string }> = {
  startingHealth: {
...
function clampRank(value: number, maxRank: number): number {
...
export function createEmptyTalentEffectTotals(): TalentEffectTotals {
  return {
    baseStats: {},
    derivedFlat: {},
    derivedPercent: {},
...
export function normalizeTalentPoints(input: unknown): Record<string, number> {
...
export function collectTalentEffectTotals(
  talentPoints: Record<string, number>,
  talentDefs: TalentNodeDef[]
): TalentEffectTotals {
...
function talentMeetsPrerequisites(
  talentPoints: Record<string, number>,
  node: TalentNodeDef
): boolean {
...
export function canPurchaseTalent(meta: MetaProgression, node: TalentNodeDef): boolean {
...
export function derivePermanentUpgradesFromTalents(talentPoints: Record<string, number>): PermanentUpgrade {
  const next: PermanentUpgrade = {
    startingHealth: 0,
    startingArmor: 0,
    luckBonus: 0,
    skillSlots: 2,
...
export function mapLegacyPermanentUpgradesToTalents(
...
export function calculateTotalShardsSpentFromTalents(
  talentPoints: Record<string, number>,
  talentDefs: TalentNodeDef[]
): number {
  let total = 0;
...
export function estimateLegacyShardsSpentFromTalents(talentPoints: Record<string, number>): number {
  let total = 0;
...
export function purchaseTalent(meta: MetaProgression, node: TalentNodeDef): MetaProgression {
  if (!canPurchaseTalent(meta, node)) {
    return meta;
  }

  const nextTalentPoints: Record<string, number> = {
    ...meta.talentPoints,
