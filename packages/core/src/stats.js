...
export function deriveStats(base, equippedItems, buffEffects, permanentUpgrades, talentEffects) {
    ...
    if (talentEffects !== undefined) {
        for (const [key, value] of Object.entries(talentEffects.derivedFlat)) {
            next[key] += value;
        }
        for (const [key, value] of Object.entries(talentEffects.derivedPercent)) {
            next[key] *= 1 + value;
        }
    }
    if (buffEffects !== undefined) {
        for (const [key, value] of Object.entries(buffEffects.additive)) {
            next[key] += value;
        }
        for (const [key, value] of Object.entries(buffEffects.multiplicative)) {
            next[key] *= value;
        }
