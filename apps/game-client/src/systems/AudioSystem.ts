...
export class AudioSystem {
  ...
  playCombatHit(isCrit: boolean): void {
    this.play(isCrit ? "sfx_combat_crit_01" : "sfx_combat_hit_01", VOLUME.sfx);
  }

  playCombatDeath(): void {
    this.play("sfx_combat_death_01", VOLUME.sfx);
  }

  playBossPhaseChange(): void {
    this.play("sfx_boss_phase_change_01", VOLUME.sfx);
  }

  playSkillUse(skillId: string): void {
    const key = SKILL_SFX_MAP[skillId];
    if (key !== undefined) {
      this.play(key, VOLUME.sfx);
    }
  }

  playConsumableUse(consumableId: ConsumableId): void {
    this.play(CONSUMABLE_SFX_MAP[consumableId], VOLUME.sfx);
  }

  playHazardTrigger(hazardType: HazardType): void {
    this.play(HAZARD_SFX_MAP[hazardType], VOLUME.sfx);
  }
