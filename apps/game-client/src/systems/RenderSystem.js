...
export class RenderSystem {
    ...
    tileHeight;
    entityDepthOffset;
    constructor(scene, tileWidth, tileHeight, entityDepthOffset) {
        this.scene = scene;
        this.tileWidth = tileWidth;
        this.tileHeight = tileHeight;
        this.entityDepthOffset = entityDepthOffset;
    }
    computeWorldBounds(dungeon) {
        const corners = [
    ...
    configureCamera(camera, worldBounds, follow) {
    ...
    spawnMonster(state, archetype, origin) {
    ...
    spawnLootSprite(item, position, origin) {
        const iso = gridToIso(position.x, position.y, this.tileWidth, this.tileHeight, origin.x, origin.y);
    ...
    syncPlayerSprite(sprite, position, yOffset, origin) {
        const iso = gridToIso(position.x, position.y, this.tileWidth, this.tileHeight, origin.x, origin.y);
        sprite.setPosition(iso.x, iso.y - yOffset);
        sprite.setDepth(iso.y + this.entityDepthOffset);
    }
    syncMonsterSprites(monsters, origin) {
        for (const monster of monsters) {
            const iso = gridToIso(monster.state.position.x, monster.state.position.y, this.tileWidth, this.tileHeight, origin.x, origin.y);
