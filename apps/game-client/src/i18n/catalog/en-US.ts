import {
  BIOME_MAP,
  BOSS_DEFS,
  BLUEPRINT_DEFS,
  ITEM_DEFS,
  ITEM_SET_DEFS,
  MONSTER_AFFIX_DEFS,
  MONSTER_ARCHETYPES,
  MUTATION_DEFS,
  RANDOM_EVENT_DEFS,
  TALENT_DEFS,
  UNLOCK_DEFS,
  SKILL_DEFS
} from "@blodex/content";
import type { MessageCatalog } from "../types";
import {
  contentAffixDescriptionKey,
  contentAffixNameKey,
  contentBiomeNameKey,
  contentBossNameKey,
  contentBlueprintNameKey,
  contentEventChoiceDescriptionKey,
  contentEventChoiceNameKey,
  contentEventDescriptionKey,
  contentEventNameKey,
  contentItemNameKey,
  contentItemSetNameKey,
  contentMutationNameKey,
  contentMonsterNameKey,
  contentSkillDescriptionKey,
  contentSkillNameKey,
  contentTalentDescriptionKey,
  contentTalentNameKey,
  contentUnlockDescriptionKey,
  contentUnlockNameKey
} from "../content/contentKeys";

const UI_MESSAGES: Record<string, string> = {
  "ui.common.close": "Close",
  "ui.common.leave": "Leave",
  "ui.common.continue": "Continue",
  "ui.common.ready": "Ready",
  "ui.common.locked": "Locked",
  "ui.common.available": "Available",
  "ui.common.status": "Status",
  "ui.common.hotkey": "Hotkey",
  "ui.common.charges": "Charges",
  "ui.common.target": "Target",
  "ui.common.mode": "Mode",
  "ui.common.floor": "Floor",
  "ui.common.kills": "Kills",
  "ui.common.loot": "Loot",
  "ui.common.obol": "Obol",
  "ui.common.time": "Time",
  "ui.common.level": "Level",
  "ui.common.score": "Score",
  "ui.common.goal": "Goal",

  "ui.consumable.health_potion.name": "Health Potion",
  "ui.consumable.health_potion.description": "Restore 40% max HP.",
  "ui.consumable.mana_potion.name": "Mana Potion",
  "ui.consumable.mana_potion.description": "Restore 60% max Mana.",
  "ui.consumable.scroll_of_mapping.name": "Scroll of Mapping",
  "ui.consumable.scroll_of_mapping.description": "Reveal current floor objective.",
  "ui.consumable.scroll_of_mapping_plus.name": "Greater Mapping Scroll",
  "ui.consumable.scroll_of_mapping_plus.description": "Reveal the floor objective and restore 25% max Mana.",
  "ui.consumable.frenzy_tonic.name": "Frenzy Tonic",
  "ui.consumable.frenzy_tonic.description": "Gain attack power and attack speed for 8 seconds.",
  "ui.consumable.phantom_brew.name": "Phantom Brew",
  "ui.consumable.phantom_brew.description": "Gain move speed and critical chance for 6 seconds.",

  "ui.hud.meta.runs": "Runs",
  "ui.hud.meta.best_floor": "Best F",
  "ui.hud.meta.best_time": "Best T",
  "ui.hud.player.title": "Vanguard",
  "ui.hud.player.hp": "HP",
  "ui.hud.player.mana": "Mana",
  "ui.hud.player.xp": "XP",
  "ui.hud.player.level": "Lvl",
  "ui.hud.player.power": "Pow",
  "ui.hud.player.armor": "Arm",
  "ui.hud.player.crit": "Crit",
  "ui.hud.player.level_up": "LEVEL UP · Lv {level}",
  "ui.hud.run.title": "Run State",
  "ui.hud.run.floor": "Floor",
  "ui.hud.run.mode": "Mode",
  "ui.hud.run.biome": "Biome",
  "ui.hud.run.status": "Status",
  "ui.hud.run.kills": "Kills",
  "ui.hud.run.loot": "Loot",
  "ui.hud.run.obol": "Obol",
  "ui.hud.run.goal": "Goal",
  "ui.hud.run.status.dead": "Dead",
  "ui.hud.run.status.hunting": "Hunting",
  "ui.hud.run.goal.stairs_up": "Stairs up",
  "ui.hud.run.goal.hunt": "Hunt",
  "ui.hud.run.mode.daily": "DAILY",
  "ui.hud.run.mode.abyss_base": "ABYSS",
  "ui.hud.run.mode.abyss": "ABYSS {floor}",
  "ui.hud.run.mapping_hint": "Mapping scroll active: objective location revealed.",
  "ui.hud.run.mutators": "Mutators: {mutators}",
  "ui.hud.run.critical_hint": "Critical HP: drink potion or disengage.",
  "ui.hud.status.buffs": "Status Rail",
  "ui.hud.status.build": "Build State",
  "ui.hud.status.build_empty": "No long-form build state yet.",
  "ui.hud.status.set_detail": "{pieces}/{total} · {active}pc",
  "ui.hud.status.buff.frost_slow": "Frost Slow",
  "ui.hud.status.buff.war_cry": "War Cry",
  "ui.hud.status.buff.guaranteed_crit": "Guaranteed Crit",
  "ui.hud.status.buff.frenzy_tonic": "Frenzy Tonic",
  "ui.hud.status.buff.phantom_brew": "Phantom Brew",
  "ui.hud.death.title": "You Died",
  "ui.hud.death.subtitle": "The Abyss claims another run.",

  "ui.hud.inventory.title": "Inventory",
  "ui.hud.inventory.subhead": "Backpack ({count})",
  "ui.hud.inventory.empty": "No drops yet.",
  "ui.hud.inventory.equip": "Equip {item}",
  "ui.hud.inventory.unequip": "Unequip {slot}",
  "ui.hud.inventory.discard": "Discard {item}",
  "ui.hud.inventory.need_level": "Need level {level}",
  "ui.hud.inventory.need_level_short": "Lv{level}",
  "ui.hud.inventory.equip_lock_hint": "Need Lv{level}",
  "ui.hud.inventory.equip_downgrade": "Equip {item} (lower score {delta})",
  "ui.hud.inventory.downgrade_hint": "Lower score {delta}",
  "ui.hud.inventory.slot.weapon.short": "WPN",
  "ui.hud.inventory.slot.helm.short": "HELM",
  "ui.hud.inventory.slot.chest.short": "CHEST",
  "ui.hud.inventory.slot.boots.short": "BOOTS",
  "ui.hud.inventory.slot.ring.short": "RING",
  "ui.hud.inventory.slot.weapon.long": "Weapon",
  "ui.hud.inventory.slot.helm.long": "Helm",
  "ui.hud.inventory.slot.chest.long": "Chest",
  "ui.hud.inventory.slot.boots.long": "Boots",
  "ui.hud.inventory.slot.ring.long": "Ring",

  "ui.hud.log.title": "System Log",
  "ui.hud.log.empty": "No events yet.",
  "ui.hud.tooltip.consumable_default_name": "Consumable",
  "ui.hud.tooltip.skill_default_name": "Skill",
  "ui.hud.tooltip.hotkey": "Hotkey: {hotkey}",
  "ui.hud.tooltip.charges": "Charges: {charges}",
  "ui.hud.tooltip.status": "Status: {status}",
  "ui.hud.tooltip.mana_cost": "Mana Cost: {manaCost}",
  "ui.hud.tooltip.base_cd": "Base CD: {seconds}s",
  "ui.hud.tooltip.target": "Target: {target}",
  "ui.hud.tooltip.slot": "Slot: {slot}",
  "ui.hud.tooltip.req_level": "Req Lvl: {level}",
  "ui.hud.tooltip.not_enough_mana": "Not enough mana.",
  "ui.hud.tooltip.no_affixes": "No affixes",
  "ui.hud.tooltip.compare": "Compare: {name}",
  "ui.hud.tooltip.power_delta": "Power Δ {delta}",
  "ui.hud.set.transition.activate": "{pieces} · activate {threshold}",
  "ui.hud.set.transition.progress": "{pieces} · next {threshold}",
  "ui.hud.set.transition.break": "{pieces} · lose {threshold}",
  "ui.hud.tooltip.summary.offense": "Offense",
  "ui.hud.tooltip.summary.defense": "Defense",
  "ui.hud.tooltip.summary.utility": "Utility",
  "ui.feedback.compare.title": "Compare Equipment",
  "ui.feedback.compare.subtitle": "Make the upgrade call while the drop still matters.",
  "ui.feedback.compare.equip_now": "Equip Now",
  "ui.feedback.compare.later": "Later",
  "ui.feedback.compare.ignore": "Ignore",
  "ui.feedback.compare.source.auto_pickup": "Dropped Loot",
  "ui.feedback.compare.source.merchant_purchase": "Merchant Purchase",
  "ui.feedback.compare.source.event_reward": "Event Reward",
  "ui.feedback.compare.source.boss_reward": "Boss Reward",
  "ui.feedback.compare.source.challenge_reward": "Challenge Reward",
  "ui.feedback.compare.source.hidden_room_reward": "Hidden Room Reward",
  "ui.feedback.compare.source.pair_fallback": "Guaranteed Spike",
  "ui.feedback.toast.rare_drop.title": "Rare Drop",
  "ui.feedback.toast.unique_drop.title": "Unique Drop",
  "ui.feedback.toast.build_formed.title": "Build Formed",
  "ui.feedback.toast.boss_reward.title": "Boss Reward Claimed",
  "ui.feedback.toast.boss_reward.detail": "Your reward is locked in. Re-check your build axis now.",
  "ui.feedback.toast.synergy_activated.title": "Synergy Activated",
  "ui.feedback.toast.elite_kill.title": "Elite Down",
  "ui.feedback.toast.elite_kill.detail": "A high-pressure target just collapsed.",
  "ui.feedback.toast.elite_kill.detail_named": "{enemy} just collapsed. The lane is open.",
  "ui.feedback.toast.near_death_reversal.title": "Clutch Recovery",
  "ui.feedback.toast.near_death_reversal.detail": "You survived the danger window and stabilized the run.",
  "ui.feedback.toast.power_spike.title": "Power Spike",
  "ui.feedback.toast.power_spike.detail": "A real upgrade window just landed.",
  "ui.feedback.toast.major_power_spike.title": "Major Power Spike",
  "ui.feedback.toast.major_power_spike.detail": "Your run just changed shape.",
  "ui.feedback.build_tag.offense": "Offense",
  "ui.feedback.build_tag.defense": "Defense",
  "ui.feedback.build_tag.utility": "Utility",
  "ui.feedback.build_tag.set": "Set Lane",
  "ui.feedback.build_tag.branching": "Branching",
  "ui.feedback.build_tag.kill_boss": "Boss Ready",
  "ui.feedback.build_tag.kill_elite": "Elite Pressure",
  "ui.feedback.build_tag.strength": "Strength",
  "ui.feedback.build_tag.dexterity": "Dexterity",
  "ui.feedback.build_tag.vitality": "Vitality",
  "ui.feedback.build_tag.intelligence": "Intelligence",
  "ui.feedback.build_tag.element_physical": "Physical",
  "ui.feedback.build_tag.element_arcane": "Arcane",
  "ui.feedback.build_tag.element_fire": "Fire",
  "ui.feedback.build_tag.element_cold": "Cold",
  "ui.feedback.build_tag.element_lightning": "Lightning",
  "ui.feedback.synergy.syn_staff_chain_lightning_overload.title": "Overload Online",
  "ui.feedback.synergy.syn_staff_chain_lightning_overload.detail": "Staff and Chain Lightning now hit with a stronger burst lane.",
  "ui.feedback.synergy.syn_dagger_shadow_step_ambush.title": "Ambush Online",
  "ui.feedback.synergy.syn_dagger_shadow_step_ambush.detail": "Dagger and Shadow Step sharpen your opener window.",
  "ui.feedback.synergy.syn_frost_spirit_resonance.title": "Frost Resonance",
  "ui.feedback.synergy.syn_frost_spirit_resonance.detail": "Frost Nova and Spirit Burst now reinforce each other.",
  "ui.feedback.synergy.syn_quake_hammer_shock.title": "Shockline Online",
  "ui.feedback.synergy.syn_quake_hammer_shock.detail": "Hammer and Quake Strike push a heavier slam pattern.",
  "ui.feedback.synergy.syn_keen_berserk_velocity.title": "Velocity Spike",
  "ui.feedback.synergy.syn_keen_berserk_velocity.detail": "Keen Eye and Berserk Echo accelerate the whole run.",
  "ui.feedback.synergy.syn_cdr_chain_window.title": "Chain Window",
  "ui.feedback.synergy.syn_cdr_chain_window.detail": "Cooldown reduction opens a tighter Chain Lightning loop.",
  "ui.hud.affix.maxHealth": "Max Health",
  "ui.hud.affix.maxMana": "Max Mana",
  "ui.hud.affix.armor": "Armor",
  "ui.hud.affix.attackPower": "Attack Power",
  "ui.hud.affix.critChance": "Crit Chance",
  "ui.hud.affix.attackSpeed": "Attack Speed",
  "ui.hud.affix.moveSpeed": "Move Speed",
  "ui.hud.affix.lifesteal": "Lifesteal",
  "ui.hud.affix.critDamage": "Crit Damage",
  "ui.hud.affix.aoeRadius": "AoE Radius",
  "ui.hud.affix.skillBonusDamage": "Skill Bonus Damage",
  "ui.hud.affix.thorns": "Thorns",
  "ui.hud.affix.healthRegen": "Health Regen",
  "ui.hud.affix.dodgeChance": "Dodge Chance",
  "ui.hud.affix.xpBonus": "XP Bonus",
  "ui.hud.affix.soulShardBonus": "Soul Shard Bonus",
  "ui.hud.affix.cooldownReduction": "Cooldown Reduction",
  "ui.hud.rarity.common": "COMMON",
  "ui.hud.rarity.magic": "MAGIC",
  "ui.hud.rarity.rare": "RARE",
  "ui.hud.weapon_type.sword": "Sword",
  "ui.hud.weapon_type.axe": "Axe",
  "ui.hud.weapon_type.dagger": "Dagger",
  "ui.hud.weapon_type.staff": "Staff",
  "ui.hud.weapon_type.hammer": "Hammer",
  "ui.hud.weapon_type.sword_master": "Sword Master",

  "ui.hud.cooldown.label": "Cooldown {seconds}s",
  "ui.hud.cooldown.ready": "Ready",

  "ui.hud.targeting.self": "Self",
  "ui.hud.targeting.nearest_enemy": "Nearest enemy",
  "ui.hud.targeting.nearest_enemy_range": "Nearest enemy ({range} range)",
  "ui.hud.targeting.around_you": "Around you",
  "ui.hud.targeting.around_you_radius": "Around you ({radius} radius)",
  "ui.hud.targeting.directional": "Directional",
  "ui.hud.targeting.directional_range": "Directional ({range} range)",
  "ui.hud.targeting.unknown": "-",

  "ui.skillbar.status.locked": "Locked",
  "ui.skillbar.status.ready": "Ready",
  "ui.skillbar.slot_status": "{charges} left · {cooldown}",
  "ui.skillbar.use_title": "Use {name}",
  "ui.skillbar.locked_slot_title": "Locked skill slot",
  "ui.skillbar.locked_slot_name": "Locked",
  "ui.skillbar.locked_slot_description": "Unlock more skill slots from meta progression.",

  "ui.summary.title.victory": "Run Victory",
  "ui.summary.title.defeat": "Run Defeat",
  "ui.summary.mode": "Mode",
  "ui.summary.floor": "Floor",
  "ui.summary.kills": "Kills",
  "ui.summary.loot": "Loot",
  "ui.summary.obol": "Obol",
  "ui.summary.score": "Score",
  "ui.summary.time": "Time",
  "ui.summary.level": "Level",
  "ui.summary.phase6.choices": "Choices",
  "ui.summary.phase6.spikes": "Spikes / Builds",
  "ui.summary.phase6.rhythm": "Rhythm",
  "ui.summary.phase6.rhythm_value": "{casts} casts / 30s · {autoShare}% auto · {idleGap}ms idle",
  "ui.summary.soul_shards": "Soul Shards",
  "ui.summary.continue": "Continue",
  "ui.summary.mode.daily": "DAILY",
  "ui.summary.failure": "Run Diagnosis",
  "ui.summary.missed": "Missed Opportunities",
  "ui.summary.next_run": "Next Run Plans",
  "ui.summary.plan_a": "Plan A",
  "ui.summary.plan_b": "Plan B",
  "ui.summary.failure.victory": "Current axis worked. Push a cleaner rerun instead of resetting the build.",
  "ui.summary.failure.boss": "Boss pressure closed the run before your build stabilized.",
  "ui.summary.failure.defense": "The run collapsed on the defense axis before damage mattered.",
  "ui.summary.failure.offense": "Damage ceiling was too low, so fights dragged into attrition.",
  "ui.summary.failure.early": "Early floor tempo slipped before a stable build formed.",
  "ui.summary.failure.attrition": "The run bled out through repeated pressure instead of one spike.",
  "ui.summary.missed.defense": "You never locked a durable layer such as armor, regen, or vitality scaling.",
  "ui.summary.missed.offense": "You did not commit to a clear damage lane, so upgrades stayed diluted.",
  "ui.summary.missed.key_drop": "No key pickup converted the run into a real power spike.",
  "ui.summary.missed.branching": "Too few high-value branches were taken, so the route stayed linear.",
  "ui.summary.missed.boss": "The boss was reached without enough burst or sustain to finish the phase check.",
  "ui.summary.missed.none": "No obvious miss was detected. The next rerun should optimize execution and routing.",
  "ui.summary.suggestion.stabilize.title": "Hold the current build together",
  "ui.summary.suggestion.stabilize.reason": "The run already shows a direction; the next step is to remove the weakest link.",
  "ui.summary.suggestion.stabilize.action": "Repeat the same core axis and spend the first two floors tightening one weakness only.",
  "ui.summary.suggestion.defense.title": "Stabilize through defense",
  "ui.summary.suggestion.defense.reason": "Your run lacked a defensive spine, so every bad trade was permanent.",
  "ui.summary.suggestion.defense.action": "Prioritize VIT, armor, health regen, and safer boots/helm slots before chasing extra damage.",
  "ui.summary.suggestion.offense.title": "Commit to one damage lane",
  "ui.summary.suggestion.offense.reason": "The run spread upgrades too thin and never formed a real kill threshold.",
  "ui.summary.suggestion.offense.action": "Lock one main stat and one weapon identity early, then stack matching affixes instead of mixed utility.",
  "ui.summary.suggestion.loot.title": "Force a key-item spike",
  "ui.summary.suggestion.loot.reason": "You survived long enough, but the run never found a defining item.",
  "ui.summary.suggestion.loot.action": "Bias elite kills, event rewards, and merchant turns until one high-value item lands.",
  "ui.summary.recommendation.defense_gap.title": "Patch the defense lane",
  "ui.summary.recommendation.defense_gap.reason": "This run never established a real defense tag.",
  "ui.summary.recommendation.defense_gap.action": "Open the next run with VIT or gear carrying armor and sustain affixes.",
  "ui.summary.recommendation.offense_gap.title": "Patch the offense lane",
  "ui.summary.recommendation.offense_gap.reason": "This run never committed to a clear offense identity.",
  "ui.summary.recommendation.offense_gap.action": "Choose one STR/DEX/INT lane early and lock a matching primary damage skill.",
  "ui.summary.recommendation.branch_low.title": "Create more build branches",
  "ui.summary.recommendation.branch_low.reason": "This run only produced {count} meaningful branch events.",
  "ui.summary.recommendation.branch_low.action": "Prioritize merchant, event, and level-up branches over a linear clear path.",
  "ui.summary.recommendation.key_item_miss.title": "Increase key-item conversion",
  "ui.summary.recommendation.key_item_miss.reason": "This run never landed a defining pickup.",
  "ui.summary.recommendation.key_item_miss.action": "Raise clear density and route into elites plus other high-value rooms first.",
  "ui.summary.recommendation.stabilize_build.title": "Keep the build coherent",
  "ui.summary.recommendation.stabilize_build.reason": "The current build tags are healthy and no single gap stands out.",
  "ui.summary.recommendation.stabilize_build.action": "Keep stacking same-axis affixes next run instead of reopening the build.",
  "ui.summary.recommendation.set_commit.title": "Lean into the active set shell",
  "ui.summary.recommendation.set_commit.reason": "A set lane has already started to form, so the next run can consolidate around it.",
  "ui.summary.recommendation.set_commit.action": "Prefer the matching set slot or same-element route instead of restarting from a neutral pickup.",
  "ui.damage_type.physical": "physical",
  "ui.damage_type.arcane": "arcane",
  "ui.damage_type.fire": "fire",
  "ui.damage_type.cold": "cold",
  "ui.damage_type.lightning": "lightning",
  "ui.summary.suggestion.route.title": "Pivot through {route}",
  "ui.summary.suggestion.route.action_branch": "At floor 2, deliberately take {route} and spend branches on event or merchant rooms instead of a straight clear.",
  "ui.summary.suggestion.route.action_default": "Route the next run toward {route} and let the biome bias reshape your equipment curve.",
  "ui.summary.route.molten": "Molten Route",
  "ui.summary.route.frozen": "Frozen Route",
  "ui.summary.route_reason.molten": "Molten favors weapon and ring spikes, which is better when you need faster damage conversion.",
  "ui.summary.route_reason.frozen": "Frozen favors helm and boots stability, which is better when you need survivability and cleaner fights.",

  "ui.progression.levelup.title": "Level Up - Attribute Allocation",
  "ui.progression.levelup.description": "Choose 1 attribute point. Pending points: {pendingPoints}.",
  "ui.progression.levelup.choice.strength.name": "Power Drill (+1 STR)",
  "ui.progression.levelup.choice.strength.description": "Raise attack scaling and stabilize close-range kill pace.",
  "ui.progression.levelup.choice.dexterity.name": "Tempo Edge (+1 DEX)",
  "ui.progression.levelup.choice.dexterity.description": "Raise crit consistency, attack speed, and movement tempo.",
  "ui.progression.levelup.choice.vitality.name": "Iron Blood (+1 VIT)",
  "ui.progression.levelup.choice.vitality.description": "Raise max health and armor for steadier survivability.",
  "ui.progression.levelup.choice.intelligence.name": "Arcane Focus (+1 INT)",
  "ui.progression.levelup.choice.intelligence.description": "Raise mana ceiling and improve skill-rotation uptime.",
  "ui.progression.levelup.preview.attack_power": "ATK {delta}",
  "ui.progression.levelup.preview.crit_chance": "CRIT {delta}",
  "ui.progression.levelup.preview.attack_speed": "AS {delta}",
  "ui.progression.levelup.preview.max_health": "HP {delta}",
  "ui.progression.levelup.preview.armor": "ARM {delta}",
  "ui.progression.levelup.preview.max_mana": "MANA {delta}",
  "ui.progression.levelup.preview.move_speed": "MOVE {delta}",
  "ui.progression.levelup_skill.title": "Level Up - Skill Choice",
  "ui.progression.levelup_skill.description": "Choose 1 skill offer. Pending skill choices: {pendingChoices}. Current slots: {slotCapacity}/5.",
  "ui.progression.levelup_skill.choice.new": "{skillName} (New)",
  "ui.progression.levelup_skill.choice.upgrade": "{skillName} (Lv.{level})",
  "ui.progression.levelup_skill.choice.meta": "CD {cooldown}s | Mana {mana}",
  "ui.progression.levelup_skill_replace.title": "Level Up - Replace Skill",
  "ui.progression.levelup_skill_replace.description": "Your current slots are full. Choose which slot to replace with {skillName}.",
  "ui.progression.levelup_skill_replace.choice": "Slot {slot}: {skillName} (Lv.{level})",
  "ui.progression.source.event": "event",
  "ui.progression.source.merchant": "merchant",
  "ui.progression.source.levelup": "level-up",
  "ui.progression.source.budget_fallback": "budget fallback",
  "ui.progression.source.levelup_skill": "level-up skill",

  "ui.event.close": "Close",
  "ui.event.leave": "Leave",
  "ui.event.merchant.title": "Wandering Merchant",
  "ui.event.merchant.subtitle": "Spend Obol to buy items for this run.",
  "ui.event.merchant.buy": "Buy ({price})",
  "ui.event.merchant.sold_out": "Sold out.",
  "ui.event.choice.unavailable": "Unavailable.",
  "ui.event.choice.need_cost": "Need {amount} {type}.",
  "ui.event.choice.invalid_cost": "invalid cost",
  "ui.event.choice.need_cost_short": "need {amount} {type}",
  "ui.boss.victory.choice.claim.name": "Claim Victory",
  "ui.boss.victory.choice.claim.description": "End run and secure rewards.",
  "ui.boss.victory.choice.enter_abyss.name": "Enter Abyss",
  "ui.boss.victory.choice.enter_abyss.description":
    "Continue to endless floors with escalating danger.",
  "ui.boss.victory.choice.enter_abyss.disabled_daily": "Daily mode does not support Abyss.",
  "boss.story.bone_throne_finale.title": "Bone Throne Cleared",
  "boss.story.bone_throne_finale.description": "Claim victory now and secure the throne reward.",
  "boss.story.bone_throne_finale.description_abyss":
    "Claim victory now or descend into the Abyss for endless escalation.",
  "boss.story.bone_throne_finale.description_daily": "Daily mode only allows Claim Victory.",
  "boss.story.bone_throne_finale.log_defeated": "Bone Sovereign defeated. Choose your fate.",
  "boss.branch.ember_warden_trial.title": "Ember Warden Routed",
  "boss.branch.ember_warden_trial.description": "Claim victory and secure the molten route reward.",
  "boss.branch.ember_warden_trial.description_abyss": "Claim victory and secure the molten route reward.",
  "boss.branch.ember_warden_trial.description_daily": "Claim victory and secure the molten route reward.",
  "boss.branch.ember_warden_trial.log_defeated": "Ember Warden routed. Claim the molten reward cache.",
  "boss.branch.cathedral_judge_trial.title": "Cathedral Judge Broken",
  "boss.branch.cathedral_judge_trial.description": "Claim victory and secure the frozen route reward.",
  "boss.branch.cathedral_judge_trial.description_abyss": "Claim victory and secure the frozen route reward.",
  "boss.branch.cathedral_judge_trial.description_daily": "Claim victory and secure the frozen route reward.",
  "boss.branch.cathedral_judge_trial.log_defeated": "Cathedral Judge defeated. Claim the frozen route cache.",
  "boss.challenge.ossuary_keeper_trial.title": "Ossuary Keeper Unbound",
  "boss.challenge.ossuary_keeper_trial.description": "Claim the keeper's cache and resume the run.",
  "boss.challenge.ossuary_keeper_trial.description_abyss": "Claim the keeper's cache and resume the run.",
  "boss.challenge.ossuary_keeper_trial.description_daily": "Claim the keeper's cache and resume the run.",
  "boss.challenge.ossuary_keeper_trial.log_defeated": "Ossuary Keeper defeated. Claim your cache and continue.",

  "ui.meta.title": "Blodex Meta Progression",
  "ui.meta.resources.soul_shards": "Soul Shards: {value}",
  "ui.meta.resources.echoes": "Echoes: {value}",
  "ui.meta.resources.unlocks": "Unlocks: {count}/{total}",
  "ui.meta.language.label": "Language",
  "ui.meta.language.aria_label": "Language switcher",
  "ui.meta.nav.difficulty": "Difficulty",
  "ui.meta.nav.daily": "Daily",
  "ui.meta.nav.talents": "Talents",
  "ui.meta.nav.forge": "Soul Forge",
  "ui.meta.nav.mutations": "Mutations",
  "ui.meta.nav.legacy": "Legacy",
  "ui.meta.section.saved_run": "Saved Run",
  "ui.meta.action.continue_run": "Continue Run",
  "ui.meta.action.abandon_run": "Abandon Run",
  "ui.meta.action.start_run": "Start New Run",
  "ui.meta.action.start_daily": "Start Daily Challenge",
  "ui.meta.action.unlock": "Unlock",
  "ui.meta.section.difficulty": "Difficulty",
  "ui.meta.section.daily": "Daily Challenge",
  "ui.meta.section.talents": "Talent Tree",
  "ui.meta.section.forge": "Soul Forge",
  "ui.meta.section.mutations": "Mutation Loadout",
  "ui.meta.section.unlocks": "Legacy Unlocks",
  "ui.meta.daily.mode.scored": "Scored",
  "ui.meta.daily.mode.practice": "Practice",
  "ui.meta.talent.path.core": "Core",
  "ui.meta.talent.path.warrior": "Warrior",
  "ui.meta.talent.path.ranger": "Ranger",
  "ui.meta.talent.path.arcanist": "Arcanist",
  "ui.meta.talent.path.utility": "Utility",
  "ui.meta.talent.tier": "Tier {tier}",
  "ui.meta.talent.cost": "Cost {cost}",
  "ui.meta.talent.rank": "Rank {rank}/{maxRank}",
  "ui.meta.unlock.tier": "Tier {tier}",
  "ui.meta.blueprint.category.skill": "Skill Blueprints",
  "ui.meta.blueprint.category.weapon": "Weapon Blueprints",
  "ui.meta.blueprint.category.consumable": "Consumable Blueprints",
  "ui.meta.blueprint.category.event": "Event Blueprints",
  "ui.meta.blueprint.category.mutation": "Mutation Blueprints",
  "ui.meta.mutation.category.offensive": "Offensive Mutations",
  "ui.meta.mutation.category.defensive": "Defensive Mutations",
  "ui.meta.mutation.category.utility": "Utility Mutations",
  "ui.meta.mutation.tier": "T{tier}",
  "ui.meta.mutation.selected_count": "Selected {selected}/{slots}. Click card to toggle, unlock echo mutations with Unlock button.",
  "ui.meta.hint.hotkeys": "Hotkeys: unlocks 1-0, difficulty Q/W/E, start Enter, daily D.",
  "ui.meta.hint.talents": "Purchase talents to improve baseline stats and run economy.",
  "ui.meta.hint.forge": "Discover blueprints in runs, then forge discovered plans with Soul Shards.",

  "ui.meta.difficulty.selected": "Selected",
  "ui.meta.difficulty.available": "Available",
  "ui.meta.difficulty.locked": "Locked",
  "ui.meta.difficulty.normal": "Normal",
  "ui.meta.difficulty.hard": "Hard",
  "ui.meta.difficulty.nightmare": "Nightmare",

  "ui.meta.status.unlocked": "Unlocked",
  "ui.meta.status.available": "Available",
  "ui.meta.status.need_progress": "Need Progress {progress}",
  "ui.meta.status.need_soul_shards": "Need Soul Shards",
  "ui.meta.status.max_rank": "Max Rank",
  "ui.meta.status.prerequisite_required": "Prerequisite Required",
  "ui.meta.status.forged": "Forged",
  "ui.meta.status.ready_to_forge": "Ready to Forge",
  "ui.meta.status.undiscovered": "Undiscovered",
  "ui.meta.status.selected": "Selected",
  "ui.meta.status.locked": "Locked",
  "ui.meta.status.cost_echoes": "Cost {cost} Echoes",
  "ui.meta.status.need_echoes": "Need {cost} Echoes",
  "ui.meta.status.available_next_refresh": "Available next refresh",
  "ui.meta.status.need_blueprint": "Need {blueprintId}",
  "ui.meta.status.slot_full": "Slot full",
  "ui.meta.status.conflict_selected": "Conflict with selected",
  "ui.meta.status.not_unlocked": "Not unlocked",
  "ui.meta.status.unavailable": "Unavailable",

  "ui.meta.requirement.hard": "Clear 1 Normal run",
  "ui.meta.requirement.nightmare": "Clear 1 Hard run",
  "ui.meta.requirement.normal": "Always available",

  "ui.meta.daily.status.scored_available": "Scored attempt available. Daily rewards can be claimed once.",
  "ui.meta.daily.status.practice_only": "Scored attempt already consumed today. Practice only (no score/reward).",

  "ui.meta.save.unknown_time": "Unknown time",
  "ui.meta.save.active_in_another_tab": "Run is active in another tab.",
  "ui.meta.save.ready_to_continue": "Saved run ready to continue.",
  "ui.meta.save.detail": "Floor {floor} • {difficulty} • {when}",
  "ui.meta.save.reset_notice_title": "Save format reset",
  "ui.meta.save.reset_notice_body": "A legacy run save was detected. Old progress cannot continue and has been reset to the new format.",

  "ui.meta.effect.permanent": "Permanent: {key} +{value}",
  "ui.meta.effect.skill_unlock": "Skill unlock: {skillId}",
  "ui.meta.effect.affix_unlock": "Affix unlock: {affixId}",
  "ui.meta.effect.biome_unlock": "Biome unlock: {biomeId}",
  "ui.meta.effect.event_unlock": "Event unlock: {eventId}",
  "ui.meta.blueprint.effect.skill_augment": "Enhance {skillName}: {effectList}",
  "ui.meta.blueprint.effect.part.cooldown_reduction": "Cooldown -{value}%",
  "ui.meta.blueprint.effect.part.mana_reduction": "Mana -{value}",
  "ui.meta.blueprint.effect.part.range_increase": "Range +{value}%",
  "ui.meta.blueprint.effect.part.duration_increase": "Duration +{value}%",
  "ui.meta.blueprint.effect.part.damage_increase": "Damage +{value}%",
  "ui.meta.blueprint.effect.part.guaranteed_crit": "Grants guaranteed crit",
  "ui.meta.mutation.unlock.default": "Default unlock",
  "ui.meta.mutation.unlock.blueprint": "Forge {blueprintId}",
  "ui.meta.mutation.unlock.echo": "Echo unlock ({cost})",
  "ui.meta.mutation.effect.on_kill_heal_percent": "on kill heal %",
  "ui.meta.mutation.effect.on_kill_attack_speed": "on kill attack speed",
  "ui.meta.mutation.effect.on_hit_invuln": "on hit invulnerability",
  "ui.meta.mutation.effect.on_hit_reflect_percent": "on hit reflect %",
  "ui.meta.mutation.effect.once_per_floor_lethal_guard": "once per floor lethal guard",
  "ui.meta.mutation.effect.drop_bonus": "drop bonus",
  "ui.meta.mutation.effect.move_speed_multiplier": "move speed multiplier",
  "ui.meta.mutation.effect.potion_heal_amp_and_self_damage": "potion heal amp + self damage",
  "ui.meta.mutation.effect.hidden_room_reveal_radius": "hidden room reveal radius",

  "ui.transition.enter_dungeon.title": "Enter the Dungeon",
  "ui.transition.enter_dungeon.subtitle": "{difficulty} · Floor 1",
  "ui.transition.daily.title": "Daily Challenge",
  "ui.transition.daily.subtitle": "{date} · HARD",
  "ui.transition.resume.title": "Resume Expedition",
  "ui.transition.resume.subtitle": "Floor {floor} · {difficulty}",
  "ui.transition.return_sanctum.title": "Return to Sanctum",
  "ui.transition.return_sanctum.subtitle": "Meta Progression",

  "ui.locale.english": "English",
  "ui.locale.zh_cn": "简体中文",
  "ui.language_gate.title": "Choose Language",
  "ui.language_gate.subtitle": "Pick your display language before starting the run.",
  "ui.language_gate.help": "Hotkeys: [1] English, [2] 简体中文, [Enter] Confirm.",
  "ui.language_gate.confirm": "Confirm Language",
  "ui.language_gate.option.english_hint": "Default terminology and original phrasing.",
  "ui.language_gate.option.zh_cn_hint": "Simplified Chinese interface and content labels.",

  "ui.legacy.meta.click_unlock_hint": "Click an unlock to purchase. Hotkeys: unlocks 1-0, difficulty Q/W/E.",
  "ui.debug.help.alt_h": "Show cheat commands",
  "ui.debug.help.alt_l": "Dump diagnostics snapshot to console/log",
  "ui.debug.help.alt_j": "Run lifecycle smoke reset loop (12 iterations)",
  "ui.debug.help.alt_o": "Add 30 Obol",
  "ui.debug.help.alt_c": "Grant 2 charges for all consumables",
  "ui.debug.help.alt_e": "Force spawn random event and open panel",
  "ui.debug.help.alt_m": "Open wandering merchant panel",
  "ui.debug.help.alt_k": "Clear current floor instantly",
  "ui.debug.help.alt_x": "Force player death (death feedback check)",
  "ui.debug.help.api_set_health": "Set player HP directly for HUD/feedback validation",
  "ui.debug.help.alt_floor_jump": "Jump to floor 1-5 (biome/hazard/boss checks)",
  "ui.debug.help.alt_n": "Start a fresh run",
  "ui.debug.help.api_force_challenge": "Inject a challenge room on current floor",
  "ui.debug.help.api_start_challenge": "Start challenge encounter immediately",
  "ui.debug.help.api_settle_challenge": "Force challenge success/failure",
  "ui.debug.help.api_open_boss_victory": "Open boss victory choice instantly",
  "ui.debug.help.api_enter_abyss": "Force enter abyss/endless",
  "ui.debug.help.api_next_floor": "Advance to next floor immediately",
  "ui.debug.help.api_force_synergy": "Inject loadout to activate a synergy quickly",
  "ui.debug.item.locked_ring": "Debug Sealed Ring (Lv{requiredLevel})",
  "ui.debug.item.sovereign_requiem": "Debug Sovereign Requiem",

  "log.debug.prefix": "[Debug] {message}",
  "log.debug.cheats_enabled": "Cheats enabled (?debugCheats=1). Press Alt+H for command list.",
  "log.debug.locked_item_added": "Added locked item: {itemName}. Press E to verify level-gate feedback.",
  "log.debug.forced_player_death": "Forced player death.",
  "log.debug.console_diagnostics_snapshot": "[Blodex] diagnostics snapshot",
  "log.debug.console_lifecycle_summary": "[Blodex] lifecycle stress summary",
  "log.debug.diagnostics_summary": "Diag listeners={listeners} monsters={monsters}/{livingMonsters} loot={loot}",
  "log.debug.lifecycle_stress_finished": "Lifecycle stress finished ({count} resets).",
  "log.debug.run_ended_start_new": "Run already ended; start a new run first.",
  "log.debug.run_ended": "Run already ended.",
  "log.debug.obols_added": "Added {added} Obol. Current: {current}.",
  "log.debug.consumables_granted": "Granted {charges} charges to all consumables.",
  "log.debug.event_boss_floor_blocked": "Cannot spawn floor event on boss floor.",
  "log.debug.event_definition_missing": "No matching event definition found for this floor/biome.",
  "log.debug.event_position_missing": "No valid position to place debug event.",
  "log.debug.event_spawned": "Spawned event {eventId}.",
  "log.debug.merchant_boss_floor_blocked": "Merchant is unavailable on boss floor.",
  "log.debug.merchant_definition_missing": "Debug event definition \"{eventId}\" not found.",
  "log.debug.merchant_position_missing": "No valid position to open merchant.",
  "log.debug.merchant_opened_panel": "Opened wandering merchant panel.",
  "log.debug.challenge_boss_floor_blocked": "Challenge room is unavailable on boss floor.",
  "log.debug.challenge_room_unavailable": "No room available for challenge injection.",
  "log.debug.challenge_injection_failed": "Challenge room injection failed.",
  "log.debug.challenge_ready": "Challenge room ready ({waves} waves).",
  "log.debug.challenge_finished": "Challenge already finished on this floor.",
  "log.debug.challenge_started": "Challenge encounter started.",
  "log.debug.challenge_active": "Challenge encounter already active.",
  "log.debug.challenge_settled": "Challenge already settled.",
  "log.debug.challenge_forced": "Challenge forced to {result}.",
  "log.debug.challenge_result.success": "success",
  "log.debug.challenge_result.failure": "failure",
  "log.debug.boss_victory_requires_boss_floor": "Boss victory choice is only available on boss floor.",
  "log.debug.boss_victory_opened": "Boss victory choice opened.",
  "log.debug.daily_cannot_enter_abyss": "Daily mode cannot enter abyss.",
  "log.debug.already_in_abyss": "Already in abyss/endless.",
  "log.debug.abyss_requires_floor_five": "Abyss entry requires reaching floor {floor}.",
  "log.debug.abyss_entered": "Forced abyss entry at floor {floor}.",
  "log.debug.floor_advanced": "Advanced to floor {floor}.",
  "log.debug.floor_advanced_endless": "Advanced to floor {floor} (endless {endlessFloor}).",
  "log.debug.unsupported_synergy_preset": "Unsupported synergy preset: {synergyId}.",
  "log.debug.synergy_skills_unavailable": "Player skills are unavailable; cannot inject synergy preset.",
  "log.debug.synergy_forced": "Forced synergy preset {synergyId}. Active: {activeList}.",
  "log.debug.none": "none",
  "log.debug.boss_runtime_not_ready": "Boss runtime not ready.",
  "log.debug.boss_health_zero_victory": "Boss health set to 0. Triggering victory summary.",
  "log.debug.floor_cleared_instantly":
    "Cleared floor instantly ({monstersRemoved} monsters removed, {drops} drops, +{levelsGained} levels).",
  "log.debug.floor_invalid_index": "Invalid floor index: {floor}",
  "log.debug.floor_already_current": "Already on floor {floor}.",
  "log.debug.floor_jumped": "Jumped to floor {floor}.",
  "log.debug.health_set": "Set HP to {current}/{max}.",
  "log.run.resumed_saved": "Resumed saved run.",
  "log.system.feedback_degraded": "Feedback degraded for {type}; continuing run safely.",
  "log.item.equip_failed_missing": "Equip failed: item {itemId} not found in backpack.",
  "log.item.equip_failed_locked": "Cannot equip {itemName}: Need Lv{requiredLevel}, current Lv{currentLevel}.",
  "log.item.equip_failed_generic": "Equip failed: {itemName} could not be equipped.",
  "log.item.discard_failed_missing": "Discard failed: item {itemId} not found in backpack.",
  "log.item.discarded": "Discarded {itemName}.",
  "log.run.objective_mapped": "Objective mapped on HUD.",
  "log.run.daily_practice_switched": "Daily scored attempt already consumed today. Switched to Practice mode.",
  "log.run.daily_scored_unlocked": "Daily scored attempt unlocked.",
  "log.combat.player_hit": "{source} hit {target} for {amount} damage.",
  "log.combat.hit": "{source} hit {target} for {amount} damage.",
  "log.combat.critical_hit": "{source} critically hit {target} for {amount} damage.",
  "log.combat.dodge": "{target} dodged {source}'s attack.",
  "log.combat.slain": "{target} was slain by {source}.",
  "log.combat.death_reason": "Slain by {source} ({amount} {damageType} damage).",
  "log.loot.drop": "{source} dropped {itemName}.",
  "log.loot.pickup": "Picked up {itemName}.",
  "log.player.level_up": "Level up! {player} reached Lv{level}.",
  "log.item.equipped": "Equipped {itemName} ({slot}).",
  "log.item.unequipped": "Unequipped {itemName} ({slot}).",
  "log.skill.used": "Skill used: {skillId}.",
  "log.skill.cooldown": "{skillId} cooldown {seconds}s.",
  "log.consumable.health_potion_used": "Health potion restored {amount} HP. Charges left: {remainingCharges}.",
  "log.consumable.mana_potion_used": "Mana potion restored {amount} mana. Charges left: {remainingCharges}.",
  "log.consumable.scroll_mapping_used": "Scroll of Mapping revealed objective. Charges left: {remainingCharges}.",
  "log.consumable.scroll_mapping_plus_used":
    "Greater mapping scroll revealed the floor and restored {amount} mana. Charges left: {remainingCharges}.",
  "log.consumable.frenzy_tonic_used": "Frenzy tonic activated. Charges left: {remainingCharges}.",
  "log.consumable.phantom_brew_used": "Phantom brew activated. Charges left: {remainingCharges}.",
  "log.consumable.cannot_use": "Cannot use {consumableId}: {reason}",
  "log.consumable.reason.no_charges": "No charges left.",
  "log.consumable.reason.hp_full": "HP already full.",
  "log.consumable.reason.mana_full": "Mana already full.",
  "log.consumable.reason.cooldown": "Cooldown {seconds}s.",
  "log.event.discovered": "Event discovered on floor {floor}: {eventName}.",
  "log.event.choice_selected": "Event choice selected: {eventId} -> {choiceId}.",
  "log.merchant.opened": "Merchant opened on floor {floor} with {offerCount} offers.",
  "log.merchant.purchased": "Purchased {itemName} for {priceObol} Obol.",
  "log.monster.state_changed": "{monsterName} state: {from} -> {to}.",
  "log.monster.affix_applied": "{monsterName} gained affix: {affixId}.",
  "log.monster.split": "{monsterName} split into {count} fragments.",
  "log.monster.leech": "{monsterName} leeched {amount} HP from {target}.",
  "log.monster.mana_burn": "{monsterName} burned {amount} mana from {target}.",
  "log.run.started": "Run started on floor {floor} ({difficulty}, seed {runSeed}).",
  "log.run.completed_victory": "Run complete! Floor {floor}, level {level}.",
  "log.run.completed_defeat": "Run ended on floor {floor}, level {level}.",
  "log.run.diagnosis": "Run diagnosis: {headline}",
  "log.run.missed_opportunity": "Missed opportunity: {message}",
  "log.run.next_plan": "Next run plan {index}: {title} - {action}",
  "log.floor.entered": "Entered floor {floor}.",
  "log.floor.entered_biome": "Entered floor {floor} ({biomeName}).",
  "log.floor.cleared": "Floor {floor} cleared with {kills} kills.",
  "log.boss.phase_shifted": "Boss shifted to phase {phase} ({hpPercent}% HP).",
  "log.boss.summoned_minions": "Boss summoned {count} minions.",
  "log.hazard.entered_zone": "Entered {hazardType} zone.",
  "log.hazard.left_zone": "Left {hazardType} zone.",
  "log.hazard.triggered": "{hazardType} triggered.",
  "log.hazard.damage": "{target} took {amount} damage from {hazardType} ({remainingHealth} HP left).",
  "log.hazard.type.damage_zone": "damage zone",
  "log.hazard.type.movement_modifier": "movement modifier",
  "log.hazard.type.periodic_trap": "periodic trap",
  "log.run.daily_loadout_active": "Daily loadout active: {weaponName}.",
  "log.hidden_room.revealed_click": "Cracked wall opened. Hidden room revealed.",
  "log.hidden_room.revealed_mutation": "Mutation pulse revealed a hidden entrance.",
  "log.challenge.discovered": "Challenge room discovered (Waves: {waves}).",
  "log.challenge.ready_confirm": "Challenge room ready. Confirm entry to start.",
  "log.challenge.wave_started": "Challenge wave {waveNumber}/{waveTotal} started.",
  "log.challenge.cleared_rewards": "Challenge cleared. Rewards dropped at the room center.",
  "log.challenge.failed_lost_hp": "Challenge failed. Lost {hpPenalty} HP.",
  "log.challenge.resumed": "Resumed active challenge encounter.",
  "log.event.encountered": "Event encountered: {eventName}.",
  "log.event.left_without_interaction": "Left event {eventName} without interaction.",
  "log.event.choice_cannot_choose": "Cannot choose {choiceName}: {reason}.",
  "log.event.cost_paid": "Event cost paid ({eventId}/{choiceId}): {amount} {costType}.",
  "log.event.reward.health": "{source}: restored {amount} HP.",
  "log.event.reward.mana": "{source}: restored {amount} mana.",
  "log.event.reward.obol": "{source}: gained {amount} Obol.",
  "log.event.reward.xp": "{source}: gained {amount} XP.",
  "log.event.reward.mapping": "{source}: mapping scroll revealed objective.",
  "log.event.reward.consumable": "{source}: gained {amount}x {consumableId}.",
  "log.event.reward.item_table_missing": "{source}: no valid item reward table.",
  "log.event.reward.item_roll_failed": "{source}: item roll failed.",
  "log.event.reward.item_acquired": "{source}: acquired {itemName}.",
  "log.event.penalty.health": "{source}: lost {amount} HP.",
  "log.event.penalty.death_reason": "Fell to event penalty ({amount} health).",
  "log.event.penalty.mana": "{source}: lost {amount} mana.",
  "log.event.penalty.obol": "{source}: lost {amount} Obol.",
  "log.event.penalty.ignored": "{source}: penalty ignored ({rewardType}).",
  "log.event.choice_not_found": "Event choice {choiceId} not found.",
  "log.event.source": "Event {eventName}",
  "log.event.backlash_source": "{eventName} backlash",
  "log.merchant.pool_missing": "Merchant pool missing.",
  "log.merchant.offer_unavailable": "Offer {offerId} unavailable.",
  "log.merchant.not_enough_obol": "Not enough Obol for {itemDefId}.",
  "log.merchant.delivery_failed": "Merchant failed to deliver {itemDefId}.",
  "log.merchant.sold_out": "Merchant sold out.",
  "log.pathfinding.aborted_unreachable": "Pathfinding aborted: cannot reach destination.",
  "log.monster.support_heal": "{sourceName} healed {targetName} for {amount}.",
  "log.run.entered_abyss_floor": "Entered Abyss Floor {floor}.",
  "log.blueprint.discovered": "Blueprint discovered ({sourceLabel}): {blueprintName}.",
  "log.progression.branch_unlocked": "Floor {floor} build branch unlocked via {source}.",
  "log.progression.branch_fallback_injected": "Floor {floor} lacked major branching; injected 1 bonus stat choice.",
  "log.progression.levelup_panel_opened": "Level-up choice panel opened ({pendingPoints} pending).",
  "log.progression.levelup_choice": "Level-up choice: {choiceName} ({pendingPoints} pending).",
  "log.progression.levelup_skill_panel_opened": "Level-up skill panel opened ({pendingChoices} pending).",
  "log.progression.levelup_skill_replace_panel_opened": "Skill replacement panel opened for {skillId} ({pendingChoices} pending).",
  "log.progression.levelup_skill_choice": "Level-up skill choice: {skillId} ({pendingChoices} pending).",
  "log.debug.kill_player.death_reason": "Debug cheat forced death to validate death feedback pipeline."
};

function buildContentMessages(): Record<string, string> {
  const messages: Record<string, string> = {};

  for (const item of ITEM_DEFS) {
    messages[contentItemNameKey(item.id)] = item.name;
  }

  for (const itemSet of ITEM_SET_DEFS) {
    messages[contentItemSetNameKey(itemSet.id)] = itemSet.name;
  }

  for (const skill of SKILL_DEFS) {
    messages[contentSkillNameKey(skill.id)] = skill.name;
    messages[contentSkillDescriptionKey(skill.id)] = skill.description;
  }

  for (const eventDef of RANDOM_EVENT_DEFS) {
    messages[contentEventNameKey(eventDef.id)] = eventDef.name;
    messages[contentEventDescriptionKey(eventDef.id)] = eventDef.description;
    for (const choice of eventDef.choices) {
      messages[contentEventChoiceNameKey(eventDef.id, choice.id)] = choice.name;
      messages[contentEventChoiceDescriptionKey(eventDef.id, choice.id)] = choice.description;
    }
  }

  for (const unlock of UNLOCK_DEFS) {
    messages[contentUnlockNameKey(unlock.id)] = unlock.name;
    messages[contentUnlockDescriptionKey(unlock.id)] = unlock.description;
  }

  for (const talent of TALENT_DEFS) {
    messages[contentTalentNameKey(talent.id)] = talent.name;
    messages[contentTalentDescriptionKey(talent.id)] = talent.description;
  }

  for (const blueprint of BLUEPRINT_DEFS) {
    messages[contentBlueprintNameKey(blueprint.id)] = blueprint.name;
  }

  for (const mutation of MUTATION_DEFS) {
    messages[contentMutationNameKey(mutation.id)] = mutation.name;
  }

  for (const biome of Object.values(BIOME_MAP)) {
    messages[contentBiomeNameKey(biome.id)] = biome.name;
  }

  for (const monster of MONSTER_ARCHETYPES) {
    messages[contentMonsterNameKey(monster.id)] = monster.name;
  }

  for (const affix of MONSTER_AFFIX_DEFS) {
    messages[contentAffixNameKey(affix.id)] = affix.name;
    messages[contentAffixDescriptionKey(affix.id)] = affix.description;
  }

  for (const boss of BOSS_DEFS) {
    messages[contentBossNameKey(boss.id)] = boss.name;
  }

  return messages;
}

export const EN_US_CATALOG: MessageCatalog = {
  locale: "en-US",
  messages: {
    ...UI_MESSAGES,
    ...buildContentMessages()
  }
};
